...
- **Steam SDK x64 via Steamworks.NET** *(last Phase 4 item)* — moved out of §3.11. Full recipe in `migration-plan-phase2.md` "Deferred Final Step" appendix. 6-method external surface (`Initialize`, `IsInitialized`, `RequestStats`, `AchievementUnlocked`, `ActivateWebOverlay`, `Shutdown`); AppID 220660 already in `game/steam_appid.txt`. Drop vendored x86 `GARSteamManager.dll` + `steam_api.dll`; rely on Steamworks.NET's `steam_api64.dll`.
